...
const [nameInput, phoneNumberInput, addressInput, zipCodeInput] =
    ...
    try {
        ...
        for (let i = 0; i < userInfo.length; i++) {
            if (userInfo[i].address === " (상세주소)") {
                ...
                userInfo[i].address.split(` (상세주소)`)[1] === undefined ||
                ...
                userInfo[
                    i
                ].address = `${addressArray[0]}<br>(상세주소) ${addressArray[1]}`;
            }

            if (
                userInfo[i].phoneNumber === undefined ||
            ...
            modifyBtn.addEventListener(
                ...
                function () {
                    ...
                    userInfoModifyBtn.addEventListener("click", async () => {
                        try {
                            const postResponse = await instance.put(
                                `api/adminUser/${userInfo[i].shortId}`,
                                {
